...
def create_soil_moisture_pipeline() -> Pipeline:
    """
    Minimal preprocessing pipeline.
    The dataset is already numeric, encoded, and standardized.
    """
...
def prepare_soil_moisture_data():
    """
    Loads train/test CSV files, renames columns to readable names,
    splits train into train/dev, loads the saved soil temperature model,
    generates predicted_soil_temp, and returns:
    ...
    soil_temp_model = load(model_dir / "soil_temperature_model.joblib")

    train_pred_soil_temp = soil_temp_model.predict(X_train)
    dev_pred_soil_temp = soil_temp_model.predict(X_dev)
    test_pred_soil_temp = soil_temp_model.predict(X_test)
    ...
    X_test["predicted_soil_temp"] = test_pred_soil_temp
